...
Locally the same three boxes run as `web`, `api`, and `db` containers from `docker compose up`. Nothing else: no Redis, no queue, no worker. The PRD forbids adding those for show, and the design does not need them (see 04 for why Postgres alone serialises the seat race).
...
sequenceDiagram
    ...
    A->>D: SELECT REQUESTED requests in Banani ORDER BY created_at
    loop each request (Rafiq, Shirin, ...)
        A->>D: joinPool if destination compatible and seats fit
    end
    A->>D: COMMIT
...
│   │   │   ├── zones/
│   │   │   ├── fare/             pure functions: haversine, quote; FareController (estimate)
...
│   └── web/                      Next.js
│       ├── src/app/              routes only
│       ├── src/components/       AppShell, StatusBadge, AsyncState, EmptyState, ColdStartBanner, ui/ (shadcn)
...
├── docker-compose.yml
├── .env.example
├── docs/                         architecture.md, erd.md, scaling.md (copied from this book as needed)
├── pnpm-workspace.yaml
└── README.md
```
