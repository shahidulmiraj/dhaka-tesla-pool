...
Reasoning, not boxes: what breaks first, what we change, what we deliberately keep. The MVP (see [architecture.md](architecture.md)) is one stateless NestJS API in front of one PostgreSQL, with the seat invariant enforced by a conditional UPDATE and a CHECK constraint.
...
| **DB contention** | The MVP already avoids one contention trap: the driver-accept sweep locks candidates one at a time with `FOR UPDATE SKIP LOCKED`, so two drivers sweeping one zone skip each other's rows instead of deadlocking. Hot pool rows: optimistic `version` column with retry, or matcher ownership (above) so only one writer touches a pool. Partition `ride_events` by month; archive to a warehouse. Connection pooling (PgBouncer / Neon pooler) in front of the primary. | Contention is only real with thousands of joins per second on one vehicle |
